<div align="center">

# 👋 Olá, eu sou Lucas Barreto!

### 👨‍💻 Analista de Implantação | Desenvolvedor

[![GitHub](https://img.shields.io/badge/GitHub-lcsbarretto-181717?style=for-the-badge\&logo=github)](https://github.com/lcsbarretto)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Lucas%20Barreto-0A66C2?style=for-the-badge\&logo=linkedin\&logoColor=white)](https://www.linkedin.com/in/lucas-barreto-magalh%C3%A3es-a265a51a1/)

</div>

---

## 🚀 Sobre mim

Sou **Analista de Implantação**, atuando com implantação, configuração e sustentação de sistemas, integrações entre aplicações, bancos de dados e troubleshooting de ambientes.

Minha rotina envolve investigar problemas, analisar logs, configurar ambientes, realizar integrações e encontrar soluções para diferentes cenários de implantação.

Também utilizo desenvolvimento e automação como ferramentas para **resolver problemas reais e otimizar processos**, criando soluções que reduzem tarefas manuais e facilitam atividades técnicas.

🎓 **Análise e Desenvolvimento de Sistemas**

💡 Atualmente, tenho interesse principalmente em:

* 🔧 Implantação e sustentação de sistemas
* 🔗 Integração entre aplicações
* 🗄️ Banco de dados
* 🤖 Automação de processos
* 🐍 Desenvolvimento com Python
* ☕ Java e integrações
* 🌐 APIs REST e SOAP
* 📄 XML e JSON
* 🐛 Troubleshooting e análise de logs
* ⚙️ Configuração de ambientes

---

## 🛠️ Tecnologias & Ferramentas

### 💻 Desenvolvimento

<p>
<img src="https://skillicons.dev/icons?i=python,java,php,js,html,css" />
</p>

### 🗄️ Banco de Dados

<p>
<img src="https://skillicons.dev/icons?i=mysql,oracle" />
<img src="https://img.shields.io/badge/SQL%20Server-CC2927?style=for-the-badge&logo=microsoftsqlserver&logoColor=white" />
</p>

### 🔗 APIs & Integrações

<p>

![REST](https://img.shields.io/badge/REST-02569B?style=for-the-badge)
![SOAP](https://img.shields.io/badge/SOAP-005571?style=for-the-badge)
![JSON](https://img.shields.io/badge/JSON-000000?style=for-the-badge\&logo=json\&logoColor=white)
![XML](https://img.shields.io/badge/XML-FF6600?style=for-the-badge)

</p>

### ⚙️ Ferramentas

<p>
<img src="https://skillicons.dev/icons?i=git,github,linux,windows,docker" />
</p>

![Postman](https://img.shields.io/badge/Postman-FF6C37?style=for-the-badge\&logo=postman\&logoColor=white)
![Selenium](https://img.shields.io/badge/Selenium-43B02A?style=for-the-badge\&logo=selenium\&logoColor=white)

---

## 🤖 Automação

Uma das áreas que mais gosto é transformar tarefas repetitivas em **processos automatizados**.

Utilizo principalmente Python para desenvolver ferramentas envolvendo:

* 🌐 Automação de sistemas web
* 🤖 Automação de tarefas
* 🗄️ Comunicação com bancos de dados
* 📊 Manipulação de dados
* 📄 Processamento de XML e JSON
* 🔌 Integração com APIs
* 🧪 Testes e validações automatizadas
* 🛠️ Ferramentas para suporte e implantação

---

## 📌 Projetos

### 🔍 Validador de Telas

Projeto desenvolvido para automatizar a validação de telas e fluxos de sistemas.

O objetivo é reduzir verificações manuais e facilitar a identificação de problemas durante testes e validações.

**Tecnologias:** Python • Selenium • Automação Web

---

### 🤖 SuricatoBot

Bot desenvolvido para auxiliar equipes de suporte na consulta de documentação e procedimentos técnicos.

A solução organiza informações técnicas e facilita o acesso aos materiais necessários durante o atendimento.

**Tecnologias:** Python • Telegram Bot API • JSON

---

### 🖥️ Scriptcase Monitor

Ferramenta voltada para testes e monitoramento de telas desenvolvidas em Scriptcase.

**Tecnologias:** Automação Web • Testes • Scriptcase

---

## 📊 GitHub Stats

<div align="center">

<img height="180em" src="https://github-readme-stats.vercel.app/api?username=lcsbarretto&show_icons=true&theme=tokyonight&include_all_commits=true&count_private=true"/>

<img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=lcsbarretto&layout=compact&langs_count=8&theme=tokyonight"/>

</div>

---

## 🔥 GitHub Streak

<div align="center">

<img src="https://streak-stats.demolab.com?user=lcsbarretto&theme=tokyonight&hide_border=false" />

</div>

---

## 🐍 My Contributions

<div align="center">

![Snake animation](https://raw.githubusercontent.com/Platane/snk/output/github-contribution-grid-snake.svg)

</div>

---

## 📈 Atividade

<div align="center">

![Activity Graph](https://github-readme-activity-graph.vercel.app/graph?username=lcsbarretto\&theme=tokyo-night)

</div>

---

<div align="center">

### 💻 "Automatize o que puder. Entenda o que não puder."

<br>

![Profile Views](https://komarev.com/ghpvc/?username=lcsbarretto\&color=blueviolet\&style=for-the-badge)

</div>
